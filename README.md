...
| **Network** | IPv4 **and IPv6** subnet calculator (CIDR, VLSM splitting, "what prefix fits 300 hosts", IPv6 scope and interface identifier, EUI-64), adapter inventory, connectivity chain test, port checks, subnet sweep, DNS lookups, listening sockets. |
...
| **Security** | SSH key generation, file hashing and signature inspection, VirusTotal lookups by hash, a cryptographic password and passphrase generator, a breach check using k-anonymity, and a security audit of 33 controls drawn from the CIS and ANSSI workstation baselines: an Essential or a Full pass, a score out of 100 weighted by risk, the antivirus or EDR actually protecting the machine named rather than Defender assumed, expected administrator accounts excludable and named in the report, and an action on every warning: a correction where a single step is safe, otherwise the settings page where the change is made, always after a confirmation that says what will happen. BitLocker is checked on every drive, with how each one unlocks. It runs elevated only, because half of what it reads is invisible to a standard user. |
...
| **Reference** | A 26 topic knowledge base, and a cross-vendor command reference of 272 commands across 16 platforms: Cisco, Cisco Meraki, Aruba AOS-CX and AOS-S, Fortinet, Palo Alto, Stormshield, pfSense and OPNsense, Juniper, Extreme, HPE Comware, MikroTik, Ubiquiti, Windows, Linux and Linux firewalling. Each has its own page in the Reference section of the navigation. |
